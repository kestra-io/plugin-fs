...
public class TriggerTest extends AbstractFileTriggerTest {
    ...
    @Test
    void maxFilesMustLimitMovedFiles() throws Exception {
        String runId = FriendlyId.createFriendlyId();

        String fromDir = "/upload/files/" + runId + "/";
        String archiveDir = "/upload/archive/" + runId + "/";
        ...
        Trigger trigger = Trigger.builder()
            ...
            .username(USERNAME)
            .password(PASSWORD)
            .maxFiles(Property.ofValue(25))
            .from(Property.ofValue(fromDir))
            .action(Property.ofValue(Downloads.Action.MOVE))
            .moveDirectory(Property.ofValue(archiveDir))
            .build();

        // Upload 100 files
        for (int i = 0; i < 100; i++) {
            utils().upload(fromDir + FriendlyId.createFriendlyId() + ".yml");
        }
        ...
        // Count files physically moved
        java.util.List<File> archivedFiles = utils().list(archiveDir).getFiles();

        assertThat(archivedFiles.size(), is(25));
    }
}
